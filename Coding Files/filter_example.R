...
# x = x1+x2+x3
x = x1+x2+x3+7*rnorm(100)
x = x1+x2+x3+30*rnorm(100)# add more noise
par(mfrow=c(2,2))
...
P =abs(2*fft(x)/100)^2 #karman filter
f=0:50/100
plot(f,P[1:51], type ="o", xlab="Frequency", ylab ="Periodogram") #if noise is too high, it's very difficult to recover the signal
# in the time domain, can't make anything out - in frequency domain - identify the singals and reverse-engineer my way bakc
